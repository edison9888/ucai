...
@implementation FlightBookSucceedCouponChoiceViewController

@synthesize flightBookSucceedViewController = _flightBookSucceedViewController;
...
- (void)dealloc
{
    ...
    [self.flightBookSucceedViewController release];
...
- (void)decidedButtonPress{
    ...
    if ([couponNoReturnString length]>0) {
        self.flightBookSucceedViewController.couponUsedContentLabel.text = [couponNoReturnString substringToIndex:[couponNoReturnString length]-1];
    }else{
        self.flightBookSucceedViewController.couponUsedContentLabel.text = @"";
    }
    if ([couponIdReturnString length]>0) {
        self.flightBookSucceedViewController.couponUsedIDs = [couponIdReturnString substringToIndex:[couponIdReturnString length]-1];
    }else{
        self.flightBookSucceedViewController.couponUsedIDs = @"";
    }
    [couponNoReturnString release];
    [couponIdReturnString release];
    
    self.flightBookSucceedViewController.couponPriceAmount = self.couponPriceAmountContentLabel.text;
    [self.flightBookSucceedViewController.flightOrderInfoTableView reloadData];
    
    int lineNumber = [CommonTools calculateRowCountForUTF8:self.flightBookSucceedViewController.couponUsedContentLabel.text bytes:20];
    self.flightBookSucceedViewController.couponUsedShowLabel.frame = CGRectMake(15, 10, 80, 20*(lineNumber==0?1:lineNumber));
    self.flightBookSucceedViewController.couponUsedContentLabel.frame = CGRectMake(95, 10, 180, 20*lineNumber);
    
    [self.flightBookSucceedViewController.payButton setFrame:CGRectMake(10, self.flightBookSucceedViewController.flightOrderInfoTableView.contentSize.height+10, 300, 40)];
    [self.flightBookSucceedViewController.flightOrderInfoTableView setContentOffset:CGPointMake(0, self.flightBookSucceedViewController.flightOrderInfoTableView.contentSize.height-330)];
    [self.navigationController popViewControllerAnimated:YES];
}
