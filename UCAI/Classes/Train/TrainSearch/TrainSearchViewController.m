...
- (void)trainSearch{
    ...
    NSString *url = [NSString stringWithFormat:@"http://www.ucai.com:8080/PhonePort/train/trainSearch?fromCity=%@&toCity=%@&sDate=%@&num=1",fromCityName, toCityName, self.startedDateLabel.text];
...
- (void)loadView {
	...
	[trainSearchButton setFrame:CGRectMake(10, 190, 300, 40)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
            ...
            case 0:
                switch (indexPath.row) {
                    case 0:{
                        ...
                        startCityTitleLabel.textAlignment = UITextAlignmentRight;
                        startCityTitleLabel.text = @"出发城市:";
                        [cell.contentView addSubview:startCityTitleLabel];
                        [startCityTitleLabel release];
                        
                        UILabel * startCityLabel = [[UILabel alloc] initWithFrame:CGRectMake(110, 10, 180, 25)];
                        ...
                        startCityLabel.text = trainDefaultStartedCityName == nil ?@"深圳":trainDefaultStartedCityName;
                    ...
                    case 1:{
                        ...
                        arrivalCityTitleLabel.textAlignment = UITextAlignmentRight;
                        arrivalCityTitleLabel.text = @"目的城市:";
                        [cell.contentView addSubview:arrivalCityTitleLabel];
                        [arrivalCityTitleLabel release];
                        
                        UILabel * arrivalCityLabel = [[UILabel alloc] initWithFrame:CGRectMake(110, 10, 180, 25)];
                        ...
                        arrivalCityLabel.text = trainDefaultArrivedCityName == nil ?@"北京":trainDefaultArrivedCityName;
                ...
                break;
            case 1:{
                UILabel *startedDateTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 10, 90, 25)];
                startedDateTitleLabel.backgroundColor = [UIColor clearColor];
                startedDateTitleLabel.textAlignment = UITextAlignmentRight;
                startedDateTitleLabel.text = @"出发日期:";
                [cell.contentView addSubview:startedDateTitleLabel];
                [startedDateTitleLabel release];
                
                UILabel * startedDateLabel = [[UILabel alloc] initWithFrame:CGRectMake(110, 10, 180, 25)];
                startedDateLabel.backgroundColor = [UIColor clearColor];
                startedDateLabel.textColor = [UIColor grayColor];
                NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
				formatter.dateFormat = @"yyyy-MM-dd";
                startedDateLabel.text = [formatter stringFromDate:[NSDate date]];
                [formatter release];
                self.startedDateLabel = startedDateLabel;
                [cell.contentView addSubview:startedDateLabel];
    ...
    if ([self.trainSearchTableView numberOfRowsInSection:indexPath.section] == 1) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.section) {
		...
		case 1:{
			NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
			formatter.dateFormat = @"yyyy-MM-dd";
            [self.datePicker setDate:[formatter dateFromString:self.startedDateLabel.text]];//设置日期滚轮的当前显示
            [formatter release];
			...
			[self.trainSearchTableView setScrollEnabled:NO];
		}
			break;
	}
