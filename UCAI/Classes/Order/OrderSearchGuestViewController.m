//
//  OrderSearchGuestViewController.m
//  JingDuTianXia
//
...
#define kHeightForSearchTableCell 44
#define kHeightForResultTableCell 65
...
@implementation OrderSearchGuestViewController
...
@synthesize contactPhoneField = _contactPhoneField;
...
@synthesize lastContractNameForSearchFlightOrderList = _lastContractNameForSearchFlightOrderList;
@synthesize lastContractPhoneForSearchFlightOrderList = _lastContractPhoneForSearchFlightOrderList;
...
- (void)dealloc{
    ...
    [self.contactPhoneField release];
    ...
    [self.flightOrderListSearchResponseModel release];
    
    [self.lastContractNameForSearchHotleOrderList release];
    [self.lastContractPhoneForSearchHotleOrderList release];
    
    [self.lastContractNameForSearchFlightOrderList release];
    [self.lastContractPhoneForSearchFlightOrderList release];
...
- (void)infoSegmentControlAction:(UIButton *)segmentControlButton
{
    switch (segmentControlButton.tag) {
        case kFlightOrderLeftSegmentControlButtonTag:
            ...
            [self.searchButton setTitle:@"查询机票订单" forState:UIControlStateNormal];
        ...
        case kHotelOrderRightSegmentControlButtonTag:
            ...
            [self.searchButton setTitle:@"查询酒店订单" forState:UIControlStateNormal];
...
- (void)textFieldNext{
    [self.contactPhoneField becomeFirstResponder];
}
...
- (void)searchButtonPress{
    ...
    [self.contactPhoneField resignFirstResponder];
	...
	if (check!=0) {
        ...
        if (self.hotelTableView.hidden) {
            ...
            ASIFormDataRequest *request = [[ASIFormDataRequest alloc] initWithURL:[NSURL URLWithString: FLIGHT_NOT_MEMBER_ORDERLIST_ADDRESS]];
...
//
- (int) checkAndSaveIn {
	if (self.contactNameField.text == nil || [@"" isEqualToString:[self.contactNameField.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]]) {
		return 1;
	} else if (self.contactPhoneField.text == nil || [@"" isEqualToString:[self.contactPhoneField.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]]) {
		return 2;
	} else if (![CommonTools verifyPhoneFormat:self.contactPhoneField.text]) {
		return 3;
	} else{
		return 0;
	}
...
- (NSData*)generateFlightOrderListSearchRequestPostXMLData:(NSString *) pageIndex {
    ...
    [pageInfoElement addChild:[GDataXMLNode elementWithName:@"pageSize" stringValue:@"5"]];
    [requestElement addChild:pageInfoElement];
    
    GDataXMLElement *conditionElement = [GDataXMLElement elementWithName:@"condition"]; 
    if ([@"1" isEqualToString:pageIndex]) {
        //第一次查询机票订单列表时
        [conditionElement addChild:[GDataXMLNode elementWithName:@"name" stringValue:self.contactNameField.text]];
        [conditionElement addChild:[GDataXMLNode elementWithName:@"mobile" stringValue:self.contactPhoneField.text]];
    } else {
        //加载机票订单列表时
        [conditionElement addChild:[GDataXMLNode elementWithName:@"name" stringValue:self.lastContractNameForSearchFlightOrderList]];
        [conditionElement addChild:[GDataXMLNode elementWithName:@"mobile" stringValue:self.lastContractPhoneForSearchFlightOrderList]];
    }
...
- (NSData*)generateHotelOrderListSearchRequestPostXMLData:(NSString *) pageIndex {
    ...
    if ([@"1" isEqualToString:pageIndex]) {
        //第一次查询酒店订单列表时
        [requestElement addChild:[GDataXMLNode elementWithName:@"linkman" stringValue:self.contactNameField.text]];
        [requestElement addChild:[GDataXMLNode elementWithName:@"mobile" stringValue:self.contactPhoneField.text]];
    } else {
        //加载酒店订单列表时
        [requestElement addChild:[GDataXMLNode elementWithName:@"linkman" stringValue:self.lastContractNameForSearchHotleOrderList]];
        [requestElement addChild:[GDataXMLNode elementWithName:@"mobile" stringValue:self.lastContractPhoneForSearchHotleOrderList]];
    }
	
    [requestElement addChild:[GDataXMLNode elementWithName:@"pageIndex" stringValue:pageIndex]];
    [requestElement addChild:[GDataXMLNode elementWithName:@"pageSize" stringValue:@"5"]];
        ...
        switch (_requestType) {
            case 1:{//酒店订单列表查询
                ...
                if (hotelOrderListSearchResponseModel.resultCode == nil || [hotelOrderListSearchResponseModel.resultCode length] == 0 || [hotelOrderListSearchResponseModel.resultCode intValue] != 1) {
                    ...
                    [_hotelRefreshHeaderView setFrame:CGRectMake(0, kHeightForResultTableCell*numberOfRowForHotelList, 320, 70)];//重设拉动刷新框的位置
                    ...
                    //标记用于记录最后查询出的酒店订单列表所用的搜索条件
                    self.lastContractNameForSearchHotleOrderList = self.contactNameField.text;
                    self.lastContractPhoneForSearchHotleOrderList = self.contactPhoneField.text;
                }
            }
                break;
            case 2:{//酒店订单列表加载更多
            ...
            case 3:{//机票订单列表查询
                ...
                if ([flightOrderListSearchResponseModel.resultCode intValue] == 1) {
                    // 提示用户打开网络联接
                    MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:self.navigationController.view];
                    hud.bgRGB = [PiosaColorManager progressColor];
                    [self.navigationController.view addSubview:hud];
                    ...
                    [_flightRefreshHeaderView setFrame:CGRectMake(0, kHeightForResultTableCell*numberOfRowForFlightList, 320, 70)];//重设拉动刷新框的位置
                    ...
                    //标记用于记录最后查询出的酒店订单列表所用的搜索条件
                    self.lastContractNameForSearchFlightOrderList = self.contactNameField.text;
                    self.lastContractPhoneForSearchFlightOrderList = self.contactPhoneField.text;
                }
            }
                break;
            case 4:{//机票订单列表加载更多
...
- (void)loadView
{
    ...
    UITableView * searchInputTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 44, 320, 100) style:UITableViewStyleGrouped];
    searchInputTableView.backgroundColor = [UIColor clearColor];
    searchInputTableView.dataSource = self;
    searchInputTableView.delegate = self;
    searchInputTableView.separatorStyle = UITableViewCellSeparatorStyleSingleLine;
    searchInputTableView.scrollEnabled = FALSE;
    searchInputTableView.tag = kSearchInputTableViewTag;
    [self.view addSubview:searchInputTableView];
    [searchInputTableView release];
    
    UIButton * tempSearchButton = [[UIButton alloc] init];
    tempSearchButton.frame = CGRectMake(10, 154, 300, 40);
    ...
    [tempSearchButton setBackgroundImage:[UIImage imageNamed:bigButtonNormalPath] forState:UIControlStateNormal];
    [tempSearchButton setBackgroundImage:[UIImage imageNamed:bigButtonHighlightedPath] forState:UIControlStateHighlighted];
    [tempSearchButton setTitle:@"查询机票订单" forState:UIControlStateNormal];
    [tempSearchButton setTitleColor:[PiosaColorManager bigMethodFontNormalColor] forState:UIControlStateNormal];
    [tempSearchButton setTitleColor:[PiosaColorManager bigMethodFontPressedColor] forState:UIControlStateHighlighted];
    [tempSearchButton addTarget:self action:@selector(searchButtonPress) forControlEvents:UIControlEventTouchUpInside];
    self.searchButton = tempSearchButton;
    [self.view addSubview:self.searchButton];
    [tempSearchButton release];
    
    UITableView * tempFlightTableView = [[UITableView alloc] initWithFrame:CGRectMake(10, 204, 300, 210) style:UITableViewStylePlain];
    ...
    UITableView * tempHotelTableView = [[UITableView alloc] initWithFrame:CGRectMake(10, 204, 300, 210) style:UITableViewStylePlain];
...
- (void)doneLoadingTableViewData:(EGORefreshTableHeaderView *) eGORefreshTableHeaderView{ 
    ...
    if (eGORefreshTableHeaderView.tag == kHotelRefreshTableHeaderViewTag) {
        ...
        //设置拉动刷新框
        int numberOfRowForHotelList = self.hotelOrderListSearchResponseModel.hotelOrderArray == nil ? 0 : [self.hotelOrderListSearchResponseModel.hotelOrderArray count];
        [_hotelRefreshHeaderView setFrame:CGRectMake(0, kHeightForResultTableCell*numberOfRowForHotelList, 320, 70)];//调整拉动刷新框的位置，使其位于刷新后的酒店列表下
        if ([self.hotelOrderListSearchResponseModel.pageSize intValue]*[self.hotelOrderListSearchResponseModel.pageIndex intValue]>=[self.hotelOrderListSearchResponseModel.orderNum intValue]) {
            //如果当前页为最后一页，则隐藏拉动刷新框
            [_hotelRefreshHeaderView setHidden:YES];
        } else {
        ...
        //设置拉动刷新框
        int numberOfRowForFlightList = self.flightOrderListSearchResponseModel.orders == nil ? 0 : [self.flightOrderListSearchResponseModel.orders count];
        [_flightRefreshHeaderView setFrame:CGRectMake(0, kHeightForResultTableCell*numberOfRowForFlightList, 320, 70)];//调整拉动刷新框的位置，使其位于刷新后的酒店列表下
        if ([self.flightOrderListSearchResponseModel.pageSize intValue]*[self.flightOrderListSearchResponseModel.currentPage intValue]>=[self.flightOrderListSearchResponseModel.count intValue]) {
            //如果当前页为最后一页，则隐藏拉动刷新框
            [_flightRefreshHeaderView setHidden:YES];
        } else {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView.tag == kSearchInputTableViewTag) {
        return kHeightForSearchTableCell;
    } else {
        return kHeightForResultTableCell;
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        ...
        if (tableView.tag == kSearchInputTableViewTag) {
            switch (indexPath.row) {
                case 0:
                {
                    UILabel * contactNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 7, 80, 30)];
                    contactNameLabel.backgroundColor = [UIColor clearColor];
                    contactNameLabel.textAlignment = UITextAlignmentRight;
                    contactNameLabel.text = @"联系人:";
                    [cell.contentView addSubview:contactNameLabel];
                    [contactNameLabel release];
                    
                    UITextField * tempContactNameField = [[UITextField alloc] initWithFrame:CGRectMake(90, 7, 200, 30)];
                    tempContactNameField.borderStyle =  UITextBorderStyleRoundedRect;
                    tempContactNameField.clearButtonMode = UITextFieldViewModeWhileEditing;
                    tempContactNameField.keyboardType = UIKeyboardTypeNamePhonePad;
                    tempContactNameField.returnKeyType = UIReturnKeyNext;
                    [tempContactNameField addTarget:self action:@selector(textFieldNext) forControlEvents:UIControlEventEditingDidEndOnExit];
                    self.contactNameField = tempContactNameField;
                    [cell.contentView addSubview:self.contactNameField];
                    [tempContactNameField release];
                }
                    break;
                    
                case 1:
                {
                    UILabel * contactPhoneLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 7, 80, 30)];
                    contactPhoneLabel.backgroundColor = [UIColor clearColor];
                    contactPhoneLabel.textAlignment = UITextAlignmentRight;
                    contactPhoneLabel.text = @"联系手机:";
                    [cell.contentView addSubview:contactPhoneLabel];
                    [contactPhoneLabel release];
                    
                    UITextField * tempContactPhoneField = [[UITextField alloc] initWithFrame:CGRectMake(90, 7, 200, 30)];
                    tempContactPhoneField.borderStyle =  UITextBorderStyleRoundedRect;
                    tempContactPhoneField.clearButtonMode = UITextFieldViewModeWhileEditing;
                    tempContactPhoneField.keyboardType = UIKeyboardTypeNumbersAndPunctuation;
                    tempContactPhoneField.returnKeyType = UIReturnKeyDone;
                    [tempContactPhoneField addTarget:self action:@selector(textFieldDone) forControlEvents:UIControlEventEditingDidEndOnExit];
                    self.contactPhoneField = tempContactPhoneField;
                    [cell.contentView addSubview:self.contactPhoneField];
                    [tempContactPhoneField release];
                }
                    break;
            }
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        } else if (tableView.tag == kHotelTableViewTag){
    ...
    if (tableView.tag == kFlightTableViewTag||tableView.tag == kHotelTableViewTag) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView.tag == kHotelTableViewTag) {
        ...
        hotelOrderInfoViewController.isMemberLogin = NO;
        hotelOrderInfoViewController.lastContractNameForSearchHotleOrderList = self.lastContractNameForSearchHotleOrderList;
        hotelOrderInfoViewController.lastContractPhoneForSearchHotleOrderList = self.lastContractPhoneForSearchHotleOrderList;
